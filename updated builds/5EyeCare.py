...
reminder_message = "Have a look far away from your current screen to protect your beautiful eyes"
...
def safe_show_custom_message():
    root.deiconify()
    custom_message = simpledialog.askstring("Set Message", "Enter the reminder message:")
    root.withdraw()
...
def set_custom_message():
    ...
    try:
        # Create a temporary root window for the dialog to avoid issues with visibility
        temp_root = tk.Tk()
        ...
        # Destroy the temporary root window after use
        temp_root.destroy()

        if custom_message:
            reminder_message = custom_message  # Assign the custom message to the global variable
